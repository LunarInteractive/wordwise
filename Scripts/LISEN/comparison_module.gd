...
func on_speech_recognized(speech: String) -> void:
	...
	if commands.has(speech):
		...
		var closest_command: String = find_closest_command(speech)
		if closest_command != "":
			var error_percentage: float = calculate_error_percentage(speech, closest_command)
			var score: int = calculate_score(error_percentage)
			
			check_word_difference(speech, closest_command)
...
func find_closest_command(speech: String) -> String:
	var min_distance: float = INF
	var closest_command: String = ""
	
	for command in commands:
		var distance: int = levenshtein_distance(speech, command)
		if distance < min_distance:
			min_distance = distance
			closest_command = command
	
	var allowed_distance: float = difficulty_settings[difficulty_level]["allowed_distance"]
	return closest_command if min_distance <= int(allowed_distance) else ""
